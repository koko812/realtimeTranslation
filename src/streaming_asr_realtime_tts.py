...
def speak_text(text, lang_code="en-US", voice_name="en-US-Wavenet-D"):
    ...
    try:
        ...
        for response in responses:
            ...
            if last_result.is_final:
                if lang=='zh':
                    translation = translate_text(full_text,target_lang="zh")
                else:
                ...
                print(f"🌍 → {translation}\n\n\n\n")
                if lang=='zh':
                    speak_text(translation, lang_code="cmn-CN", voice_name="cmn-CN-Chirp3-HD-Achernar")
                else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Streaming speech translation with TTS")
    parser.add_argument("--lang", choices=["en", "zh"], default="en",
                        help="Target TTS language: en (English) or zh (Mandarin Chinese)")
    args = parser.parse_args()

    # lang 引数を streaming_asr に渡す
    streaming_asr(lang=args.lang)
